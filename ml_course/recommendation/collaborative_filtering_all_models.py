...
# Count the number of movies where a genre is assigned
...
""" 
The goal is to factorise the ratings matrix A into the product of a user embedding matrix U and movie embedding matrix 
V, so A~UV^T, with U=[u1,...uN] and V=[v1,...vM]

where:
N = Number of users
M = Number of movies
Aij = rating of the jth movies by the ith user
Ui = d-dimensional vector (embedding) representing user i
Vj = d-dimensional vector (embedding) representing movie j
The prediction of the model for (i,j) pair is the dot product <Ui,Vj>

Using sparse representation:
The rating matrix could be very large and, in general, most of the entries are unobserved, since a given user will only 
rate a small subset of movies. Use tf.SparseTensor. A SparseTensor uses three tensors to represent the matrix: 
tf.SparseTensor(indices, values, dense_shape) represents a tensor, where a value Aij = a is encoded by setting
indices[k] = [i, j] and values[k] = a. The last tensor dense_shape is used to specify the shape of the full underlying 
matrix.

Example with 2 movies and 4 users:
SparseTensor(
  indices=[[0, 0], [0, 1], [1,3]],  - position in matrix
  values=[5.0, 3.0, 1.0],           - value of that position
  dense_shape=[2, 4])               - shape of rating matrix
"""
...
"""
The model approximates the ratings matrix A by a low-rank product UV^T. Mean Squared Error of observed entries only can
be used (loss function)
"""
...
"""
Next: Compute the recommendations, look at the nearest neighbors of some movies, look at the norms of the movie
embeddings, visualise the embedding in a projected embedding space
"""
DOT = 'dot'
COSINE = 'cosine'
...
"""
You can observe that the recommendations with dot-product and cosine are different: with dot-product, the model tends 
to recommend popular movies. This can be explained by the fact that in matrix factorization models, the norm of the 
embedding is often correlated with popularity (popular movies have a larger norm), which makes it more likely to 
recommend more popular items. We can confirm this hypothesis by sorting the movies by their embedding norm.
"""
...
model_lowinit = build_model(ratings, embedding_dim=30,
...
movie_embedding_norm([model, model_lowinit])  # Comparing previous model with new model

# Embedding visualization
"""
Project the embeddings to a lower dimensional space. T-SNE (T-distributed Stochastic Neighbor Embedding) is an 
algorithm that projects the embeddings while attempting to preserve their pariwise distances. It can be useful for 
visualization
"""
...
"""
The model does not know how to place the embeddings of irrelevant movies with MSE. THis is known as folding. Adding 
regularisation terms will address the issue:
    * Regularization of the model parameters. This is a common L2 regularization term on the embedding matrices
...
"""
Adding the regularization terms results in a higher MSE, both on the training and test set. However the quality of the 
recommendations improves. Fitting the observed data often emphasizes learning high similarity (between items with many 
interactions), but a good embedding representation also requires learning low similarity (between items with few or 
no interactions).
"""
...
movie_embedding_norm([model, model_lowinit, reg_model])  # Compare all
tsne_movie_embeddings(reg_model)  # Visualize the embeddings
